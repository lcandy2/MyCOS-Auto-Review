let comment = window.myCOSData?.comment ?? "我对本课程非常满意。";
let radioSelection = window.myCOSData?.radioSelection ?? [0, 1];
let checkboxSelection = window.myCOSData?.checkboxSelection ?? 1;
let autoSubmission = window.myCOSData?.autoSubmission ?? 0;
let fillComment = window.myCOSData?.fillComment ?? 1;

window.addEventListener('MyCOSData', function (e) {
  // console.log('Received MyCOS data: ', e.detail);
  window.myCOSData = e.detail;
  comment = window.myCOSData?.comment ?? comment;
  radioSelection = window.myCOSData?.radioSelection ?? radioSelection;
  checkboxSelection = window.myCOSData?.checkboxSelection ?? checkboxSelection;
  autoSubmission = window.myCOSData?.autoSubmission ?? autoSubmission;
  fillComment = window.myCOSData?.fillComment ?? fillComment;
  fetch('/config.js')
...
const executeMyCOS = () => {
  ...
  appendLog('已完成《' + $("h1").text() + '》！\n若已完成全部评教任务，为避免与其他网站冲突，请及时禁用本拓展。', 'green');
}
